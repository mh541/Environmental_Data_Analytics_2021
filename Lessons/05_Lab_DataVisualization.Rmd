...
ggsave("Users/mandyhooks/Environmental_Data_Analytics_2021/Output/Plot_text.jpg",Plot_text, height = 4, width = 6, units = "in", dpi = 300)
...
EPAair <- read.csv("/Users/mandyhooks/Environmental_Data_Analytics_2021/Data/Processed/EPAair_O3_PM25_NC1819_Processed.csv", stringsAsFactors = TRUE)
...
SurfaceTemp<-ggplot(subset(PeterPaul.chem.nutrients, depth==0), aes(x=daynum, y=temperature_C, color=year4))+
  geom_point()+
...
SurfaceTempDate<-ggplot(subset(PeterPaul.chem.nutrients), aes(x=sampledate, y=temperature_C, color=depth))+
  geom_point(size=.5)+
  scale_color_viridis(option = "magma", direction = -1)+
  facet_wrap(vars(lakename), nrow=2)+
  scale_x_date(date_breaks = "5 years") 
print(SurfaceTempDate)
...
airplot<- ggplot(EPAair, aes(x=PM2.5, y=Ozone, color=SITE_LATITUDE))+
  geom_point(alpha=.5)+
...
PM2.5plot<-ggplot(EPAair, aes(x=Year, y=PM2.5))+
  geom_histogram()+
  geom_smooth(method=lm)+
