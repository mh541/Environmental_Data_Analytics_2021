...
LakeChem<-read.csv("./Data/Processed/NTL-LTER_Lake_Chemistry_Nutrients_PeterPaul_Processed.csv", stringsAsFactors = TRUE)
GathNut<-read.csv("./Data/Processed/NTL-LTER_Lake_Nutrients_PeterPaulGathered_Processed.csv", stringsAsFactors = TRUE)
NiwotRidge<-read.csv(".//Data/Processed/NEON_NIWO_Litter_mass_trap_Processed.csv", stringsAsFactors = TRUE)

#2 

as.Date(LakeChem$sampledate)
as.Date(GathNut$sampledate)
...
#TN Boxplot
boxtn<- ggplot(LakeChem, aes(x=month, y=tn_ug))+
  geom_boxplot(aes(color=lakename))+
  ylim(120,600)+
