...
```{r fig.height = 3, fig.width = 4, warning = FALSE}
#Temperature Boxplot
boxtemp<-ggplot(LakeChem, aes(x=month, y=temperature_C, shape=as.factor(month)))+
...
#TP Boxplot
boxtp<- ggplot(LakeChem, aes(x=month, y=tp_ug, shape= as.factor(month)))+
...
```{r cowplot attempt 1}
plot_grid(boxtemp, boxtp, boxtn, align_plots('h'), ncol = 3)
```

```{r TA suggestion}
plot_grid(boxtemp, boxtp, boxtn, align="hv",ncol=3,rel_widths=c(1,1,1))
```
